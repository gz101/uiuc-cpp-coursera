...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (!head_) {
    // If empty, simply link head_ and tail_ to the new element.
    head_ = newNode;
    tail_ = newNode;
  } else {
    // Otherwise, traverse through the list and insert into its correct position.
    // Keep going until either the end of the list, or the correct position found.
    Node* curr = head_;
    while (curr && curr->data < newNode->data) {
      curr = curr->next;
    }

    // Check if should insert at the end of the list.
    if (!curr) {
      // newNode has the largest value in the list.
      tail_->next = newNode;
      newNode->prev = tail_;
      tail_ = newNode;
    } else {
      // Insert position is somewhere within the current list.
      newNode->prev = curr->prev;
      newNode->next = curr;

      // Special case if newNode should be first element in the list.
      if (curr == head_) {
        head_ = newNode;
      }

      // Only applies if node is not at the beginning of the list.
      if (curr->prev) {
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // Edge case, if either of the lists are empty.
  if (left.empty()) {
    return right;
  } else if (right.empty()) {
    return left;
  ...
  // Traverse through both lists while they are non-empty.
  ...
  // Ensure remaining elements are pushed onto the merged list.
  while (!left.empty()) {
    merged.pushBack(left.front());
    left.popFront();
  }
